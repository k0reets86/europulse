...
final class EPV2_Queue {
	...
	public static function next_item_for_processing(bool $ignore_retry_after = false): ?object {
		...
		$continuation_items = array_values(array_filter(
			self::processable_items_for_states(['retry_process', 'ready_review'], $ignore_retry_after),
			static function ($item): bool {
				return self::processing_stage_priority($item) > 0;
			}
		));
		if ($continuation_items !== []) {
			$load = self::category_load(['retry_process', 'ready_publish', 'publishing']);
			usort($continuation_items, static function ($a, $b) use ($load): int {
			...
			return $continuation_items[0] ?? null;
		}
		$new_items = self::processable_items_for_states(['new']);
		if ($new_items !== []) {
			usort($new_items, static function ($a, $b): int {
				$aBreaking = self::row_is_breaking($a) ? 1 : 0;
				$bBreaking = self::row_is_breaking($b) ? 1 : 0;
				if ($aBreaking !== $bBreaking) {
					return $bBreaking <=> $aBreaking;
				}
				$aScore = self::score_from_row((array) $a);
				$bScore = self::score_from_row((array) $b);
				if ($aScore !== $bScore) {
					return $bScore <=> $aScore;
				}
				return strcmp((string) ($a->created_at ?? ''), (string) ($b->created_at ?? ''));
			});
			return $new_items[0] ?? null;
		}

		$items = self::processable_items_for_states(['retry_process', 'ready_review'], $ignore_retry_after);
		if ($items === []) {
			return null;
		}
		$load = self::category_load(['retry_process', 'ready_publish', 'publishing']);
	...
	private static function processable_items_for_states(array $states, bool $ignore_retry_after = false): array {
		$states = array_values(array_filter(array_map('strval', $states)));
		if ($states === []) {
			return [];
		}
		$limit = in_array('new', $states, true) && count($states) === 1 ? 30 : 200;
		$items = self::get_items(['states' => $states, 'limit' => $limit]);
		return array_values(array_filter($items, static function ($item) use ($ignore_retry_after): bool {
			if (self::retry_process_has_legacy_worker_error($item)) {
				return false;
			}
	...
	public static function promote_publish_ready_payloads(array $states = ['retry_process']): int {
		...
		$items = self::get_items(['states' => $states, 'limit' => 200]);
		if ($items === []) {
			return 0;
		}
		$promoted = 0;
	...
	public static function has_processable_items(): bool {
		...
		$placeholders = implode(',', array_fill(0, count($states), '%s'));
		$sql = $wpdb->prepare(
			"SELECT id FROM {$table} WHERE state IN ({$placeholders}) ORDER BY created_at ASC LIMIT 1",
			...$states
		);
		$item_id = (int) $wpdb->get_var($sql);
		return $item_id > 0;
	}
		...
		foreach ($items as $item) {
			if (self::retry_process_has_legacy_worker_error($item)) {
				...
				self::mark_state((int) $item->id, 'retry_process', [
					...
					'error_message' => 'Материал возвращён в автоматическую очередь после восстановления legacy worker-контура.',
				]);
				$changed++;
				continue;
			}
			...
			if (self::retry_process_is_exhausted($item) || self::review_rebuild_exhausted($item)) {
				...
				self::update_fields((int) $item->id, [
					'admin_notes' => wp_json_encode($notes, JSON_UNESCAPED_UNICODE),
					'error_message' => 'Материал возвращён в полную автоматическую доработку после сброса exhausted-counters.',
				]);
				$changed++;
			}
		}
	...
	private static function retry_process_has_legacy_worker_error(object $item): bool {
		if ((string) ($item->state ?? '') !== 'retry_process') {
			return false;
		}
		$message = (string) ($item->error_message ?? '');
		return preg_match('/worker_shared_secret|undefined method\s+EPV2_Settings::worker_shared_secret/i', $message) === 1;
	}
		...
		$items = array_values(array_filter($items, static function ($item) use ($force): bool {
			...
			if ($payload !== [] && ! EPV2_AI_Processor::payload_is_publish_ready($payload)) {
				self::mark_state((int) $item->id, 'retry_process', [
					...
					'error_message' => 'Пакет снят из очереди публикации: финальный publish-gate больше не пройден.',
				]);
				return false;
			}
	...
	public static function mark_state(int $id, string $state, array $extra = []): void {
		...
		$current = $wpdb->get_row($wpdb->prepare(
			"SELECT state, admin_notes, ai_payload, category_final, category_proposed FROM {$wpdb->prefix}epv2_queue WHERE id = %d LIMIT 1",
			$id
		));
		if (! $current) {
			return;
	...
	public static function set_live_status(int $id, string $message, string $code = ''): void {
		global $wpdb;
		$notes_json = $wpdb->get_var($wpdb->prepare(
			"SELECT admin_notes FROM {$wpdb->prefix}epv2_queue WHERE id = %d LIMIT 1",
			$id
		));
		if ($notes_json === null) {
			return;
		}
		$notes = json_decode((string) $notes_json, true);
		$notes = is_array($notes) ? $notes : [];
		$notes['_system'] = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
	...
	private static function processing_stage_priority(object $row): int {
		...
		if ($state === 'retry_process' && preg_match('/featured image|featured media|изображение не подошло|главного изображения/u', $message) === 1) {
			return 520;
		}
