...
final class EPV2_Source_Enricher {
	public static function enrich_item(object $item, array $options = []): array {
		...
		$candidates = self::search_supporting_sources($item, $dossier['primary'], $context_memory);
	...
	private static function search_supporting_sources(object $item, array $primary, array $context_memory = []): array {
		$results = [];
		$seen = [];
		foreach (self::build_search_queries($item, $primary, $context_memory) as $query) {
			$lang = 'de';
			$gl = 'DE';
			$ceid = 'DE:de';
