...
final class EPV2_Worker_Client {
	...
	private static function send( array $payload ): array {
		$timeout = max( 30, min( 300, (int) EPV2_Settings::get( 'worker_timeout_seconds', 120 ) ) );
		...
		if ( $code !== 200 ) {
			...
			if ( is_array( $decoded ) ) {
				$detail = (string) ( $decoded['detail'] ?? $body );
			}
			throw new RuntimeException( "Worker returned HTTP {$code}: {$detail}" );
		}
