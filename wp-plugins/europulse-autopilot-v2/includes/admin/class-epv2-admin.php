...
if (! defined('ABSPATH')) {
	...
	public static function settings(): void {
		...
		self::row('Внешний worker', '<select name="worker_mode"><option value="disabled"' . selected((string) ($settings['worker_mode'] ?? 'disabled'), 'disabled', false) . '>выключен</option><option value="cli"' . selected((string) ($settings['worker_mode'] ?? 'disabled'), 'cli', false) . '>CLI worker</option></select><p class="description">Тяжёлая AI/media обработка выносится во встроенный PHP worker вне web-запросов.</p>');
		self::row('Worker: PHP binary', '<input name="worker_python_bin" value="' . esc_attr((string) ($settings['worker_python_bin'] ?? 'php')) . '" class="regular-text code">');
		self::row('Worker: entrypoint', '<input name="worker_cli_command" value="' . esc_attr((string) ($settings['worker_cli_command'] ?? '/var/www/europulse/worker/php_worker.php')) . '" class="regular-text code"><p class="description">Поддерживается только локальный php worker script внутри каталога worker.</p>');
		self::row('Worker: env helper', '<input name="worker_src_dir" value="' . esc_attr((string) ($settings['worker_src_dir'] ?? '')) . '" class="regular-text code"><p class="description">Legacy поле. Для встроенного PHP worker не используется.</p>');
	...
	private static function queue_state_label(object $item): string {
		...
		$progress = self::queue_progress_percent($item, $notes);
		...
		if ($live_status !== '' && in_array($state, ['processing_de', 'publishing'], true)) {
			return trim($live_status . ($progress > 0 ? ' (' . $progress . '%)' : ''));
		}
		...
		if ($progress > 0 && in_array($state, ['new', 'processing_de', 'retry_process', 'ready_review', 'retry_publish', 'ready_publish', 'publishing'], true)) {
			$label .= ' (' . $progress . '%)';
		}
		return $label;
	}

	private static function queue_progress_percent(object $item, array $notes = []): int {
		$state = (string) ($item->state ?? '');
		if (in_array($state, ['published', 'draft_created', 'pending_review', 'partially_created'], true)) {
			return 100;
		}
		...
		if ($state === 'new' || $state === 'reserve') {
			return 0;
		}

		$notes = $notes !== [] ? $notes : (json_decode((string) ($item->admin_notes ?? ''), true) ?: []);
		$system = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
		$live_code = sanitize_key((string) ($system['live_status_code'] ?? ''));

		$payload = self::queue_cached_payload($item);
		$meta = is_array($payload['_meta'] ?? null) ? $payload['_meta'] : [];
		$pipeline_stage = sanitize_key((string) ($meta['pipeline_stage'] ?? ''));

		$by_code = [
			'analyzing' => 10,
			'repairing_media' => 20,
			'enrichment_rebuild' => 30,
			'rewriting' => 40,
			'worker_full_bundle' => 45,
			'worker_rebuild' => 55,
			'generating_payload' => 60,
			'translating_variants' => 75,
			'worker_translate_finish' => 80,
			'publish_finish' => 85,
			'worker_publish_finish' => 90,
			'publishing_bundle' => 95,
			'publishing' => 98,
		];
		if (isset($by_code[$live_code])) {
			return $by_code[$live_code];
		}

		$by_stage = [
			'translate_finish' => 75,
		];
		if (isset($by_stage[$pipeline_stage])) {
			return $by_stage[$pipeline_stage];
		}

		if (in_array($state, ['processing_de', 'publishing'], true)) {
			return 15;
		}
		if (in_array($state, ['retry_process', 'ready_review'], true)) {
			return 0;
		}
		if ($state === 'retry_publish') {
			return 95;
		}
		return 0;
	}
	...
	private static function queue_action_hint(string $state): string {
		return match ($state) {
			'new' => 'Материал ждёт своей очереди на полную автоматическую обработку.',
			'processing_de' => 'Материал сейчас обрабатывается и усиливается.',
			'ready_review' => 'Материал временно отмечен внутренним review-state, но в операторской очереди показывается как новый до следующего автоматического прохода.',
			'retry_process' => 'Материал остаётся в общей очереди и снова станет единственным активным item, когда дойдёт его очередь.',
