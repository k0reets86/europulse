...
final class EPV2_Jobs {
	...
	private const OPTION_MANUAL_COLLECT = 'epv2_manual_collect_requested_at';
	private const OPTION_MANUAL_PROCESS = 'epv2_manual_process_requested_at';
	private const OPTION_MANUAL_PUBLISH = 'epv2_manual_publish_requested_at';
	...
	public static function register(): void {
		...
		add_action('admin_init', [self::class, 'maybe_schedule']);
		add_action('admin_init', [self::class, 'maybe_kick_pipeline'], 20);
	}
	...
	public static function maybe_schedule(): void {
		if (! self::should_run_web_side_effects()) {
			return;
		}
	...
	public static function enqueue_collect(): void {
		if (self::server_orchestrator_enabled()) {
			self::request_manual_stage(self::HOOK_COLLECT);
			return;
		}
	...
	public static function enqueue_process(): void {
		if (self::server_orchestrator_enabled()) {
			self::request_manual_stage(self::HOOK_PROCESS);
			return;
		}
	...
	public static function enqueue_publish(): void {
		if (self::server_orchestrator_enabled()) {
			self::request_manual_stage(self::HOOK_PUBLISH);
			return;
		}
	...
	public static function run_collect_windowed(): void {
		self::prepare_windowed_stage();
		self::recover_orphan_collect_lock();
		if (EPV2_Lock_Manager::is_active('collect')) {
			return;
		}
		$force = self::consume_manual_stage_request(self::HOOK_COLLECT);
		if (! $force && EPV2_Runs::has_recent_started('collect', self::recent_run_guard_seconds('collect'))) {
			return;
		}
		EPV2_Collector::run_scheduled($force);
	}
	...
	public static function run_process_windowed(): void {
		...
		$force = self::consume_manual_stage_request(self::HOOK_PROCESS);
		if (! $force && EPV2_Runs::has_recent_started('process', self::recent_run_guard_seconds('process'))) {
			return;
		}
		EPV2_Queue::promote_publish_ready_payloads(['retry_process', 'ready_review']);
		EPV2_Queue::normalize_ready_publish_schedule();
		if (! $force && ! EPV2_Queue::has_processable_items()) {
			if (EPV2_Queue::next_item_for_publish() && ! EPV2_Lock_Manager::is_active('publish')) {
				self::enqueue_publish();
			}
			return;
		}
		EPV2_AI_Processor::process_scheduled($force, $force);
	}
	...
	public static function run_publish_windowed(): void {
		...
		$force = self::consume_manual_stage_request(self::HOOK_PUBLISH);
		if (! $force && EPV2_Runs::has_recent_started('publish', self::recent_run_guard_seconds('publish'))) {
			return;
		}
		EPV2_Publisher::publish_scheduled($force);
	}
	...
	public static function run_process_async(): void {
		...
		EPV2_AI_Processor::process_scheduled(true, $force);
	}
	...
	private static function should_run_web_side_effects(): bool {
	...
	private static function prepare_windowed_stage(): void {
		if (self::server_orchestrator_enabled()) {
			return;
		}
	...
	private static function recent_run_guard_seconds(string $job): int {
	...
	private static function request_manual_stage(string $hook): void {
		update_option(self::manual_request_option($hook), time(), false);
	}

	private static function consume_manual_stage_request(string $hook): bool {
		$option = self::manual_request_option($hook);
		$requested_at = (int) get_option($option, 0);
		if ($requested_at <= 0) {
			return false;
		}
	...
	private static function manual_request_option(string $hook): string {
		return match ($hook) {
			self::HOOK_COLLECT => self::OPTION_MANUAL_COLLECT,
			self::HOOK_PROCESS => self::OPTION_MANUAL_PROCESS,
			self::HOOK_PUBLISH => self::OPTION_MANUAL_PUBLISH,
			default => '',
		};
	}
	...
	private static function recover_orphan_process_lock(): void {
		...
		$server_orchestrator = self::server_orchestrator_enabled();
		$stale_window = $server_orchestrator
			? max(120, min(180, (int) floor($job_lock_ttl / 6)))
			: max(300, min(600, (int) floor($job_lock_ttl / 2)));
		$stale_after = time() - $stale_window;
		$stuck_without_item_after = time() - ($server_orchestrator ? 30 : 60);
		...
		if ($has_processing_item) {
			...
			if ($requeued) {
				...
				if ($latest && (string) ($latest->status ?? '') === 'started') {
					...
					if ($started > 0 && $started <= $stale_after) {
						EPV2_Runs::finish((int) $latest->id, 'finished_with_errors', 0, 1, [
							'result' => 'stale_processing_item_recovered',
						]);
					}
				}
			}
	...
	private static function recover_orphan_collect_lock(): void {
		...
		if (! is_array($lock) || empty($lock['token'])) {
			...
			if ($latest && (string) ($latest->status ?? '') === 'started') {
				...
				if ($started > 0 && $started <= $stale_after) {
					EPV2_Runs::finish((int) $latest->id, 'finished_with_errors', 0, 1, [
						'result' => 'stuck_collect_started_without_lock_recovered',
					]);
				}
			}
			return;
	...
	private static function pending_action_count(string $hook, string $group): int {
		...
		static $tables_available = null;
		if ($tables_available === null) {
			$actions = $wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $table_actions));
			$groups = $wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $table_groups));
			$tables_available = ($actions === $table_actions && $groups === $table_groups);
		}
		if (! $tables_available) {
			return 0;
		}
