...
final class EPV2_Resilience_Manager {
	...
	public static function cleanup(): void {
		...
		self::normalize_google_wrapper_source_urls();
	...
	public static function cleanup_review_support(): void {
	...
	public static function schedule_retry(object $item, string $state, string $module, string $message): void {
		...
		if ($attempt > $maxAttempts) {
			if (self::should_keep_retrying_automatically($module, $message)) {
				...
				$retry_state = self::attach_retry_process_stage_contract($item, $retry_state, $extra);
				EPV2_Queue::mark_state((int) $item->id, $retry_state, $extra);
				return;
			}
			$system['retries'][$module] = $attempt;
			$system['last_error_human'] = $human;
		...
		$state = self::attach_retry_process_stage_contract($item, $state, $extra);
		EPV2_Queue::mark_state((int) $item->id, $state, $extra);
	}
	...
	private static function terminal_state_for_exhausted_retry(string $module, string $message, string $human): array {
		...
		if (preg_match('/устарел|устарела|lost relevance/i', $message)) {
			return [
				'state' => 'retry_process',
				'message' => 'Материал сохраняется в automation и уходит в обязательную переоценку актуальности, а не в terminal reject.',
			];
		}
		return [
			'state' => 'error',
	...
	private static function should_keep_retrying_automatically(string $module, string $message): bool {
		...
		$module = sanitize_key($module);
		return in_array($module, ['process', 'publish', 'publish_media'], true);
	}
	...
	private static function extended_retry_delay_seconds(int $attempt, string $module, string $message): int {
		if (
			...
			return match (true) {
				...
				$attempt <= 6 => 10 * MINUTE_IN_SECONDS,
				default => 15 * MINUTE_IN_SECONDS,
			};
		}
		$base = self::retry_delay_seconds(max(1, min($attempt, 6)), $module, $message);
		return max(30 * MINUTE_IN_SECONDS, min(6 * HOUR_IN_SECONDS, $base * 2));
	}
	...
	private static function attach_retry_process_stage_contract(object $item, string $state, array &$extra): string {
		if (sanitize_key($state) !== 'retry_process') {
			return $state;
		}
		$payload_json = array_key_exists('ai_payload', $extra) ? (string) $extra['ai_payload'] : (string) ($item->ai_payload ?? '');
		$payload = json_decode($payload_json, true);
		$payload = is_array($payload) ? EPV2_AI_Processor::normalize_existing_payload($payload, false) : [];
		if ($payload === []) {
			return $state;
		}
		$required_stage = EPV2_AI_Processor::payload_required_stage($payload);
		if ($required_stage !== '') {
			$payload = EPV2_AI_Processor::force_payload_pipeline_stage($payload, $required_stage);
		}
		$extra['ai_payload'] = wp_json_encode($payload, JSON_UNESCAPED_UNICODE);
		$extra['category_final'] = implode(',', array_values(array_filter((array) ($payload['categories'] ?? [])))) ?: (string) ($item->category_final ?? $item->category_proposed ?? '');
		if (class_exists('EPV2_Jobs') && EPV2_Jobs::orchestrator_v2_enabled()) {
			$state = 'new';
			$notes = self::item_notes($item);
			$notes['_system'] = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
			$notes['_system']['workflow_step'] = $required_stage !== '' ? sanitize_key($required_stage) : sanitize_key((string) ($notes['_system']['workflow_step'] ?? ''));
	...
	private static function normalize_google_wrapper_source_urls(): void {
		$rows = EPV2_Queue::get_queue_items_summary([
			'states' => ['new', 'retry_process', 'ready_review', 'error'],
			'limit' => 25,
		]);
		foreach ($rows as $item) {
			$fields = EPV2_Google_News::normalize_queue_row_urls($item);
			if ($fields === []) {
				continue;
			}
			...
			if ($payload !== []) {
				$payload['original_url'] = (string) ($fields['original_url'] ?? $item->original_url ?? '');
				$payload['_meta'] = is_array($payload['_meta'] ?? null) ? $payload['_meta'] : [];
				$payload['_meta']['original_url'] = (string) ($fields['original_url'] ?? $item->original_url ?? '');
				...
				if ($dossier !== []) {
					...
					$primary_url = trim((string) ($dossier['primary']['url'] ?? ''));
					if ($primary_url === '' || str_contains($primary_url, 'news.google.com/')) {
						$dossier['primary']['url'] = (string) ($fields['original_url'] ?? $item->original_url ?? '');
					}
	...
	private static function reclaim_stuck_items(): void {
		...
		$stuckProcess = EPV2_Queue::get_queue_items_summary(['states' => ['processing_de'], 'limit' => 50]);
		$stuckPublish = EPV2_Queue::get_queue_items_summary(['states' => ['publishing'], 'limit' => 50]);
	...
	private static function normalize_retry_metadata(): void {
		...
		foreach ($rows as $item) {
			...
			if ($payload !== []) {
				if (
					preg_match('/minimum DE master quality/i', (string) ($item->error_message ?? '')) === 1
				) {
					$payload = EPV2_AI_Processor::force_payload_pipeline_stage($payload, 'rebuild_bundle');
				}
	...
	private static function normalize_targeted_retry_windows(): void {
		$rows = EPV2_Queue::get_queue_items_summary([
			'states' => ['new', 'retry_process', 'retry_publish'],
			'limit' => 50,
		]);
		$processQueued = false;
		foreach ($rows as $item) {
			...
			if ((int) ($retries['process'] ?? 0) > 0 && preg_match('/minimum DE master quality|publish threshold|minimum review threshold|heuristic payload|broken multilingual/iu', $message) === 1) {
				$attempt = max(1, (int) ($retries['process'] ?? 1));
				$target = gmdate('Y-m-d H:i:s', time() + self::retry_delay_seconds($attempt, 'process', $message));
				$current = (string) ($system['retry_after'] ?? '');
				if ($current === '' || strtotime($current) > strtotime($target)) {
					$system['retry_after'] = $target;
					$notes['_system'] = $system;
			...
			if ((int) ($retries['process'] ?? 0) > 0 && self::is_external_worker_infra_error($message)) {
				$attempt = max(1, (int) ($retries['process'] ?? 1));
				$target = gmdate('Y-m-d H:i:s', time() + self::retry_delay_seconds($attempt, 'process', $message));
				$current = (string) ($system['retry_after'] ?? '');
				if ($current === '' || strtotime($current) < strtotime($target)) {
			...
			if (self::is_stale_rebuild_backlog_candidate($item)) {
				$target = gmdate('Y-m-d H:i:s', time() + (15 * MINUTE_IN_SECONDS));
				$current = (string) ($system['retry_after'] ?? '');
				if ($current === '' || strtotime($current) < strtotime($target)) {
					$system['retry_after'] = $target;
					$notes['_system'] = $system;
	...
	private static function wake_idle_recoverable_process_queue(): void {
		if (EPV2_Lock_Manager::is_active('process') || EPV2_Queue::has_processable_items()) {
			return;
		}
		$rows = EPV2_Queue::get_queue_items_summary([
		...
		$candidate = null;
		$candidate_retry_after = PHP_INT_MAX;
		foreach ($rows as $item) {
			$notes = self::item_notes($item);
			$system = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
			if ((string) ($system['manual_confirmation_required'] ?? '') !== '') {
				continue;
			}
			$message = (string) ($item->error_message ?? '');
			if (
				preg_match('/minimum DE master quality|publish threshold|minimum review threshold|heuristic payload|broken multilingual/iu', $message) !== 1
				&& ! self::is_external_worker_infra_error($message)
			) {
				continue;
			}
			$retry_after = (string) ($system['retry_after'] ?? '');
			$retry_ts = $retry_after !== '' ? (strtotime($retry_after) ?: PHP_INT_MAX) : 0;
			if ($retry_ts < $candidate_retry_after) {
				$candidate = $item;
				$candidate_retry_after = $retry_ts;
			}
		}
		if (! $candidate instanceof stdClass) {
			return;
		}
		$notes = self::item_notes($candidate);
		$notes['_system'] = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
		$target = gmdate('Y-m-d H:i:s', time());
		$current = (string) ($notes['_system']['retry_after'] ?? '');
		if ($current === '' || strtotime($current) > strtotime($target)) {
			$notes['_system']['retry_after'] = $target;
			EPV2_Queue::update_fields((int) $candidate->id, [
				'admin_notes' => wp_json_encode($notes, JSON_UNESCAPED_UNICODE),
			]);
		}
	...
	private static function normalize_stalled_rebuild_cooldowns(): void {
		$rows = EPV2_Queue::get_queue_items_summary([
			'states' => ['new', 'retry_process'],
			'limit' => 50,
		]);
		foreach ($rows as $item) {
			$message = (string) ($item->error_message ?? '');
			if (preg_match('/publish threshold stalled rebuild bundle/i', $message) !== 1) {
				continue;
			}
			$payload = json_decode((string) ($item->ai_payload ?? ''), true);
			$payload = is_array($payload) ? EPV2_AI_Processor::normalize_existing_payload($payload, false) : [];
			...
			$required_stage = EPV2_AI_Processor::payload_required_stage($payload);
			if ($required_stage === 'rebuild_bundle') {
				continue;
			}
			...
			$next_state = $required_stage !== '' ? 'retry_process' : 'new';
	...
	private static function normalize_manual_confirmation_queue_states(): void {
		$rows = EPV2_Queue::get_queue_items_summary([
			'states' => ['new', 'retry_process'],
			'limit' => 50,
		]);
		foreach ($rows as $item) {
			$notes = self::item_notes($item);
			$system = is_array($notes['_system'] ?? null) ? $notes['_system'] : [];
			$manual = (string) ($system['manual_confirmation_required'] ?? '');
			if ($manual === '') {
				continue;
			}
			unset($system['retry_after']);
			$system['live_status'] = '';
			$system['live_status_code'] = '';
			$notes['_system'] = $system;
			$message = trim((string) ($item->error_message ?? ''));
			if ($message === '') {
				$message = $manual === 'translation'
					? 'Требует ручного подтверждения translation: автоматическая языковая доводка не достигла валидного результата.'
					: 'Требует ручного подтверждения media: автоматическая медиа-доводка не достигла безопасного publishable результата.';
			}
			EPV2_Queue::mark_state((int) $item->id, 'ready_review', [
				'admin_notes' => wp_json_encode($notes, JSON_UNESCAPED_UNICODE),
	...
	private static function normalize_recoverable_manual_confirmations(): void {
		$rows = EPV2_Queue::get_queue_items_summary([
			'states' => ['ready_review'],
			'limit' => 50,
		]);
		foreach ($rows as $item) {
			...
			if ($manual === 'media') {
				$payload = EPV2_AI_Processor::repair_media_for_automation($item, $payload);
				if (EPV2_AI_Processor::payload_requires_media_manual_review($payload)) {
					continue;
				}
			}
			...
			$required_stage = EPV2_AI_Processor::payload_required_stage($payload);
			$next_state = $required_stage !== '' ? 'retry_process' : 'ready_review';
			EPV2_Queue::mark_state((int) $item->id, $next_state, [
				'ai_payload' => wp_json_encode($payload, JSON_UNESCAPED_UNICODE),
				'category_final' => implode(',', array_values(array_filter((array) ($payload['categories'] ?? [])))),
				'error_message' => '',
	...
	private static function revive_auto_review_candidates(): void {
		...
		foreach ($rows as $item) {
			...
			if (EPV2_AI_Processor::item_has_expired_live_angle($item)) {
				EPV2_Resilience_Manager::schedule_retry($item, 'retry_process', 'process', 'Материал требует повторной переоценки live-угла вместо terminal reject.');
				continue;
			}
			$payload = json_decode((string) ($item->ai_payload ?? ''), true);
			$payload = is_array($payload) ? $payload : [];
		...
		if ($publishQueued && ! EPV2_Jobs::server_orchestrator_enabled() && ! EPV2_Lock_Manager::is_active('publish')) {
	...
	private static function is_stale_rebuild_backlog_candidate(object $item): bool {
		if ((string) ($item->state ?? '') !== 'retry_process') {
			return false;
		}
		$payload = json_decode((string) ($item->ai_payload ?? ''), true);
		$payload = is_array($payload) ? $payload : [];
		$stage = sanitize_key((string) ($payload['_meta']['pipeline_stage'] ?? ''));
		if ($stage !== 'rebuild_bundle') {
			return false;
		}
		$message = trim((string) ($item->error_message ?? ''));
		if (preg_match('/legacy auto ready_review sink/i', $message) === 1) {
			return true;
		}
		$updated = strtotime((string) ($item->updated_at ?? '')) ?: 0;
		return $message === '' && $updated > 0 && (time() - $updated) >= 900;
	}
		...
		foreach ($rows as $item) {
			...
			if ($hasPayload && ($reviewablePayload || $score >= 60 || $reworkable)) {
				if (self::automation_is_auto()) {
					...
					unset($notes['_system']['retry_after']);
					EPV2_Queue::mark_state((int) $item->id, 'retry_process', [
						'ai_payload' => $hasPayload ? wp_json_encode($payload, JSON_UNESCAPED_UNICODE) : '',
						'category_final' => $hasPayload ? (implode(',', array_values(array_filter((array) ($payload['categories'] ?? [])))) ?: (string) ($item->category_final ?? '')) : (string) ($item->category_final ?? ''),
						'error_message' => 'Материал автоматически возвращён в доработку после stale error-state.',
						'admin_notes' => wp_json_encode($notes, JSON_UNESCAPED_UNICODE),
					]);
				} else {
					EPV2_Queue::mark_state((int) $item->id, 'ready_review', [
	...
	private static function automation_is_auto(): bool {
		return (string) EPV2_Settings::get('mode', 'semi') === 'auto';
	}
}
