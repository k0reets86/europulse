...
final class EPV2_Review {
	private const REVIEW_METRICS_TTL = 300;
	...
	public static function ensure_payload(object $item): array {
		...
		if (! self::needs_rebuild($existing)) {
			...
			return self::refresh_review_metrics($existing);
		}
	...
	public static function ensure_payload_for_review(object $item): array {
		$existing = self::decode_payload((string) ($item->ai_payload ?? ''));
		if (! self::needs_rebuild($existing)) {
			return self::prepare_review_payload($existing, false);
		}

		$payload = self::build_payload_without_ai_from_dossier(
			$item,
			self::normalize_categories((string) ($item->category_proposed ?: $item->category_final ?: 'deutschland')),
			(string) EPV2_Settings::get('rewrite_style', 'lively'),
			self::minimal_dossier_from_item($item),
			true
		);

		return self::prepare_review_payload($payload, true);
	}

	public static function ensure_payload_without_ai(object $item, array $categories, string $style = 'strict'): array {
		$dossier = EPV2_Source_Enricher::enrich_item($item, ['fast_mode' => true]);
		return self::build_payload_without_ai_from_dossier($item, $categories, $style, $dossier);
	}

	public static function build_payload_without_ai_from_dossier(object $item, array $categories, string $style, array $dossier, bool $lightweight = false): array {
		...
		if ($lightweight) {
			$payload['_meta']['quality'] = ['score' => 0, 'warnings' => ['de' => ['lightweight fallback pending full validation']]];
			$payload['_meta']['seo_quality'] = ['score' => 0, 'warnings' => ['de' => ['lightweight fallback pending full validation']]];
			$payload['_meta']['release_quality'] = ['score' => 0, 'warnings' => ['de' => ['lightweight fallback pending full validation']]];
			$payload['_meta']['google_quality'] = ['score' => 0, 'warnings' => ['de' => ['lightweight fallback pending full validation']]];
			$payload['_meta']['review_payload_mode'] = 'lightweight';
			$payload['_meta']['review_metrics_refreshed_at'] = '';
			return $payload;
		}

		return self::refresh_review_metrics($payload);
	}
	...
	public static function save_payload(int $item_id, array $payload): void {
		...
		if ($categories !== [] && (string) ($item->state ?? '') === 'published') {
	...
	private static function prepare_review_payload(array $payload, bool $lightweight): array {
		$payload = EPV2_AI_Processor::stabilize_context_memory($payload);
		$payload['_meta'] = is_array($payload['_meta'] ?? null) ? $payload['_meta'] : [];
		if (! $lightweight && self::review_metrics_snapshot_due($payload)) {
			$payload = self::refresh_review_metrics($payload);
		}
		$payload['_meta']['review_payload_mode'] = $lightweight ? 'lightweight' : 'stored';
		return $payload;
	}

	public static function refresh_review_metrics(array $payload): array {
		$payload = EPV2_AI_Processor::stabilize_context_memory($payload);
		...
		$payload['_meta']['review_metrics_refreshed_at'] = gmdate('Y-m-d H:i:s');
		if (empty($payload['_meta']['review_payload_mode'])) {
			$payload['_meta']['review_payload_mode'] = 'stored';
		}
		return EPV2_AI_Processor::normalize_existing_payload($payload, false);
	}

	private static function has_review_metrics_snapshot(array $payload): bool {
	...
	private static function review_metrics_snapshot_due(array $payload): bool {
		if (! self::has_review_metrics_snapshot($payload)) {
			return true;
		}
		$meta = is_array($payload['_meta'] ?? null) ? $payload['_meta'] : [];
		if (self::metrics_look_placeholder($meta)) {
			return true;
		}
		$refreshed_at = strtotime((string) ($meta['review_metrics_refreshed_at'] ?? '')) ?: 0;
		if ($refreshed_at <= 0) {
			return true;
		}
		return ($refreshed_at + self::REVIEW_METRICS_TTL) <= time();
	}

	private static function metrics_look_placeholder(array $meta): bool {
		foreach (['quality', 'seo_quality', 'release_quality', 'google_quality'] as $key) {
			$warnings = (array) (($meta[$key]['warnings'] ?? []));
			foreach ($warnings as $warning_group) {
				foreach ((array) $warning_group as $warning) {
					if (stripos((string) $warning, 'lightweight fallback pending full validation') !== false) {
						return true;
					}
				}
			}
	...
	private static function minimal_dossier_from_item(object $item): array {
		$url = esc_url_raw((string) ($item->original_url ?? ''));
		$image = esc_url_raw((string) ($item->source_image_url ?? ''));
		return [
			'primary' => [
				'url' => $url,
				'host' => (string) wp_parse_url($url, PHP_URL_HOST),
				'source_name' => sanitize_text_field((string) wp_parse_url($url, PHP_URL_HOST)),
				'title' => sanitize_text_field((string) ($item->original_title ?? '')),
				'excerpt' => sanitize_text_field((string) ($item->original_excerpt ?? '')),
