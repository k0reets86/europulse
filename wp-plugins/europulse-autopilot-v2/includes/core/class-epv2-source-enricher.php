...
final class EPV2_Source_Enricher {
	public static function enrich_item(object $item, array $options = []): array {
		...
		$fast_mode = ! empty($options['fast_mode']);
		$max_runtime_seconds = max(2, min(20, (int) ($options['max_runtime_seconds'] ?? ($fast_mode ? 4 : 8))));
		$force_supporting = ! empty($options['force_supporting']);
		$target_supporting = max(1, min(3, (int) ($options['target_supporting'] ?? ($fast_mode ? 1 : 2))));
		...
		if (! $fast_mode && in_array($category, ['sport', 'kultur', 'community', 'wirtschaft', 'world'], true)) {
			$should_search_supporting = true;
		}
		if ($fast_mode && ! $force_supporting) {
			$should_search_supporting = false;
		}
		if (! $should_search_supporting) {
			return $dossier;
		}

		if (self::runtime_budget_exceeded($started_at, $max_runtime_seconds)) {
			return $dossier;
		}

		$candidates = self::search_supporting_sources($item, $dossier['primary'], $context_memory, $started_at, $max_runtime_seconds);
		$dossier['used_search'] = $candidates !== [];
		...
		foreach ($candidates as $candidate) {
			...
			if (! self::candidate_entry_is_story_relevant($candidateEntry, $item, $dossier['primary'], $context_memory)) {
				continue;
			}
			...
			if (! self::candidate_entry_is_story_relevant($entry, $item, $dossier['primary'], $context_memory)) {
				continue;
			}
		...
		$dossier['supporting'] = self::filter_supporting_entries((array) ($dossier['supporting'] ?? []), $item, $dossier['primary'], $context_memory);
	...
	private static function best_supporting_primary_candidate(array $supporting): array {
		...
		usort($supporting, static function (array $left, array $right): int {
			$leftScore = (int) ($left['_support_score'] ?? 0) * 240;
			$rightScore = (int) ($right['_support_score'] ?? 0) * 240;
	...
	private static function search_supporting_sources(object $item, array $primary, array $context_memory = [], ?float $started_at = null, int $max_runtime_seconds = 8): array {
		$results = [];
		$seen = [];
		$queries = self::build_search_queries($item, $primary, $context_memory);
		foreach (array_slice($queries, 0, 3) as $query) {
			if ($started_at !== null && self::runtime_budget_exceeded($started_at, $max_runtime_seconds)) {
				return $results;
			}
			try {
				foreach (self::search_supporting_sources_bing($query) as $candidate) {
		...
		if (count($results) < 3) {
			foreach (self::search_supporting_sources_from_active_sources($item, $primary, $context_memory) as $candidate) {
				if ($started_at !== null && self::runtime_budget_exceeded($started_at, $max_runtime_seconds)) {
					return $results;
				}
				$key = md5((string) ($candidate['url'] ?? '') . '|' . (string) ($candidate['title'] ?? ''));
		...
		if (count($results) < 2) {
			foreach (array_slice($queries, 0, 2) as $query) {
				...
				try {
					foreach (self::search_supporting_sources_bing_web($query) as $candidate) {
						$key = md5((string) ($candidate['url'] ?? '') . '|' . (string) ($candidate['title'] ?? ''));
						if (isset($seen[$key])) {
							continue;
						}
	...
	private static function runtime_budget_exceeded(float $started_at, int $max_runtime_seconds): bool {
		return (microtime(true) - $started_at) >= $max_runtime_seconds;
	}

	private static function search_supporting_sources_from_active_sources(object $item, array $primary, array $context_memory = []): array {
	...
	private static function search_supporting_sources_bing(string $query): array {
		$query = trim($query);
		if ($query === '') {
			return [];
		}
	...
	private static function search_supporting_sources_bing_web(string $query): array {
		$query = trim($query);
		if ($query === '') {
			return [];
		}
	...
	private static function build_search_queries(object $item, array $primary, array $context_memory = []): array {
		...
		$memoryTitle = self::normalize_query((string) ($context_memory['event_title'] ?? ''), 8);
		$queries = [];
		if ($memoryTitle !== '') {
			$queries[] = $memoryTitle;
			$queries[] = self::normalize_query($memoryTitle . ' ' . $category, 8);
			$queries[] = self::normalize_query($memoryTitle . ' bild', 8);
		}
		foreach (array_slice((array) ($context_memory['search_terms'] ?? []), 0, 4) as $term) {
			$queries[] = self::normalize_query((string) $term, 8);
		}
		$queries[] = self::normalize_query($coreTitle, 6);
		$queries[] = self::normalize_query($title . ' ' . $category, 8);
		$queries[] = self::normalize_query($primaryTitle . ' ' . $category, 8);
		...
		$entityQuery = implode(' ', array_slice((array) ($storyContext['entities'] ?? []), 0, 3));
		if ($entityQuery !== '') {
	...
	private static function candidate_entry_is_story_relevant(array $entry, object $item, array $primary, array $context_memory = []): bool {
		$category = self::effective_category($item, $primary);
		$storyContext = self::extract_story_context_from_primary($item, $primary, $context_memory);
		...
		if (
			! self::looks_like_section_landing($entry)
			&& self::source_entry_matches_story_context($storyTitle, $storyExcerpt, $storyContent, $entryTitle, $entryExcerpt, $entryContent, [$category], $storyContext)
		) {
			return true;
		}
	...
	private static function filter_supporting_entries(array $supporting, object $item, array $primary, array $context_memory = []): array {
		$filtered = [];
		foreach ($supporting as $entry) {
			if (! is_array($entry) || ! self::candidate_entry_is_story_relevant($entry, $item, $primary, $context_memory)) {
				continue;
			}
			$filtered[] = $entry;
		}
