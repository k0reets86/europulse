...
final class EPV2_Resilience_Manager {
	...
	private const OPTION_LAST_CLEANUP = 'epv2_last_resilience_cleanup';

	public static function cleanup(): void {
		if (! self::should_run_cleanup()) {
			return;
		}
		self::mark_cleanup_run();
		$lightweight = EPV2_Jobs::server_orchestrator_enabled() && ! (defined('WP_CLI') && WP_CLI);
		$timings = [];
		$cleanup_started_at = microtime(true);
		self::measure_cleanup_step($timings, 'lock_cleanup', static function (): void {
			EPV2_Lock_Manager::cleanup();
		});
		self::measure_cleanup_step($timings, 'cleanup_abandoned_started', static function (): void {
			EPV2_Runs::cleanup_abandoned_started(120);
		});
		self::measure_cleanup_step($timings, 'normalize_stale_error_items', static function (): void {
			self::normalize_stale_error_items();
		});
		self::measure_cleanup_step($timings, 'recover_worker_infra_errors', static function (): void {
			self::recover_worker_infra_errors();
		});
		self::measure_cleanup_step($timings, 'restore_rejected_reviewable_items', static function (): void {
			self::restore_rejected_reviewable_items();
		});
		if (! $lightweight) {
			self::measure_cleanup_step($timings, 'normalize_retry_metadata', static function (): void {
				self::normalize_retry_metadata();
			});
			self::measure_cleanup_step($timings, 'normalize_terminal_review_candidates', static function (): void {
				self::normalize_terminal_review_candidates();
			});
			self::measure_cleanup_step($timings, 'normalize_targeted_retry_windows', static function (): void {
				self::normalize_targeted_retry_windows();
			});
			self::measure_cleanup_step($timings, 'hydrate_review_payloads', static function (): void {
				self::hydrate_review_payloads();
			});
			self::measure_cleanup_step($timings, 'revive_auto_review_candidates', static function (): void {
				self::revive_auto_review_candidates();
			});
		}
		if (! $lightweight) {
			self::measure_cleanup_step($timings, 'normalize_terminal_retry_process_items', static function (): void {
				EPV2_Queue::normalize_terminal_retry_process_items();
			});
		}
		self::measure_cleanup_step($timings, 'reclaim_stuck_items', static function (): void {
			self::reclaim_stuck_items();
		});
		$promoted = 0;
		self::measure_cleanup_step($timings, 'promote_publish_ready_payloads', static function () use (&$promoted): void {
			$promoted = EPV2_Queue::promote_publish_ready_payloads(['retry_process']);
		});
		if ($promoted > 0 && ! EPV2_Lock_Manager::is_active('publish')) {
			self::measure_cleanup_step($timings, 'normalize_ready_publish_schedule', static function (): void {
				EPV2_Queue::normalize_ready_publish_schedule();
			});
			self::measure_cleanup_step($timings, 'enqueue_publish', static function (): void {
				EPV2_Jobs::enqueue_publish();
			});
		}
		$total_ms = (int) round((microtime(true) - $cleanup_started_at) * 1000);
		if ($total_ms >= 5000) {
			EPV2_Logger::info('resilience', 'Slow cleanup detected', [
				'total_ms' => $total_ms,
				'promoted' => $promoted,
				'timings_ms' => $timings,
	...
	private static function should_run_cleanup(): bool {
		if (PHP_SAPI === 'cli') {
			return true;
		}
		...
		$last_run = (int) get_option(self::OPTION_LAST_CLEANUP, 0);
		return $last_run <= 0 || (time() - $last_run) >= 60;
	}

	private static function mark_cleanup_run(): void {
		update_option(self::OPTION_LAST_CLEANUP, time(), false);
	}

	private static function measure_cleanup_step(array &$timings, string $name, callable $callback): void {
		$started_at = microtime(true);
		$callback();
		$timings[$name] = (int) round((microtime(true) - $started_at) * 1000);
	}
	...
	public static function schedule_retry(object $item, string $state, string $module, string $message): void {
		...
		if ($module === 'process' && self::is_external_worker_infra_error($message)) {
			...
			$maxAttempts = max($maxAttempts, 12);
		} elseif ($module === 'publish_media') {
			$maxAttempts = max($maxAttempts, 10);
		}
	...
	private static function terminal_state_for_exhausted_retry(string $module, string $message, string $human): array {
		...
		if (
			...
			return [
				'state' => 'retry_process',
				'message' => 'Материал возвращён в расширенную автоматическую доработку до publish-grade.',
			];
		}
		if (
			...
			return [
				'state' => 'retry_process',
				'message' => 'Материал возвращён в автоматическую media/source доработку.',
			];
		}
	...
	private static function reclaim_stuck_items(): void {
		...
		$missingLockProcessAge = max(180, min(360, (int) floor($jobLockTtl / 4)));
		...
		if (! EPV2_Lock_Manager::is_active('process') && ! EPV2_Jobs::server_orchestrator_enabled()) {
	...
	private static function normalize_terminal_review_candidates(): void {
		...
		foreach ($rows as $item) {
			...
			$extra = [
				'error_message' => 'Материал возвращён в расширенную автоматическую доработку после terminal-review fallback.',
			];
			if ($payload !== []) {
				$extra['ai_payload'] = wp_json_encode($payload, JSON_UNESCAPED_UNICODE);
			}
			EPV2_Queue::mark_state((int) $item->id, 'retry_process', $extra);
		}
	...
	private static function normalize_stale_error_items(): void {
		...
		foreach ($rows as $item) {
			...
			if ($hasPayload && ($reviewablePayload || $score >= 60 || $reworkable)) {
				EPV2_Queue::mark_state((int) $item->id, 'retry_process', [
					'error_message' => 'Материал возвращён в автоматическую доработку после повторяющихся recoverable ошибок внешнего сервиса.',
				]);
				continue;
			}
			EPV2_Queue::mark_state((int) $item->id, 'rejected', [
