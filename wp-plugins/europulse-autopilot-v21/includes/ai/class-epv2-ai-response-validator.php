...
final class EPV2_AI_Response_Validator {
	...
	public static function enrich_payload(array $payload, bool $resolve_media = true): array {
		...
		$payload['featured_media_url'] = EPV2_Media::normalize_featured_candidate_url((string) ($payload['featured_media_url'] ?? $payload['media_url'] ?? ''));
		...
		$payload['media_url'] = EPV2_Media::normalize_featured_candidate_url((string) ($payload['media_url'] ?? ''));
		...
		$current_featured = (string) ($payload['featured_media_url'] ?? '');
		$keep_existing_featured =
			$current_featured !== ''
			&& EPV2_Media::can_use_featured_url($current_featured)
			&& ! self::media_url_is_blocked($payload, $current_featured);
		if ($keep_existing_featured) {
			$payload['media_url'] = $current_featured;
		} elseif ($resolve_media) {
		...
		if ($payload['featured_media_url'] !== '') {
			...
			$payload['media_origin_url'] = '';
			$payload['media_credit'] = '';
			$payload['media_caption'] = '';
			foreach (['de', 'uk', 'en'] as $lang) {
				if (is_array($payload['languages'][$lang] ?? null)) {
					$payload['languages'][$lang]['media_origin_url'] = '';
					$payload['languages'][$lang]['media_credit'] = '';
					$payload['languages'][$lang]['media_caption'] = '';
				}
			}
		}
	...
	public static function editorial_quality(array $payload): array {
		...
		foreach (['de', 'uk', 'en'] as $lang) {
			...
			$lang_warnings = [];
			$formatRules = self::publication_format_rules($payload, $lang);

			$title_words = self::word_count($title);
			if ($title_words > 0 && ($title_words < (int) ($formatRules['title_min_words'] ?? 6) || $title_words > (int) ($formatRules['title_max_words'] ?? 14))) {
				$lang_warnings[] = 'заголовок не попадает в newsroom-диапазон по длине';
				$score -= 6;
			}
			if ($title !== '' && self::headline_looks_generic($title)) {
				$lang_warnings[] = 'заголовок слишком общий и больше похож на тему, чем на новость';
				$score -= 10;
			}
			...
			$bureaucratic_terms = [
				...
				'entsprechend', 'im rahmen', 'wurde mitgeteilt', 'teilte mit', 'hinsichtlich',
				'відповідно до', 'згідно з', 'положення', 'розпорядження', 'повідомляється',
				'according to section', 'pursuant to', 'ordinance', 'official notice', 'it was stated',
			];
			$bureaucratic_hits = 0;
			...
			if (preg_match('/\b(im rahmen von|wurde .* mitgeteilt|teilte .* mit|according to the statement|in accordance with|відповідно до .* було|згідно з .* повідомляється)\b/iu', $combined) === 1) {
				$lang_warnings[] = 'текст звучит как пресс-релиз, а не как живая новость';
				$score -= 10;
			}
			...
			if ($excerpt !== '') {
				$lead_sentences = self::sentence_count($excerpt);
				if ($lead_sentences < 2 || $lead_sentences > 3) {
					$lang_warnings[] = 'лид должен состоять из 2-3 предложений';
					$score -= 8;
				}
				$lead_words = self::word_count($excerpt);
				if ($lead_words > 0 && ($lead_words < (int) ($formatRules['lead_min_words'] ?? 25) || $lead_words > (int) ($formatRules['lead_max_words'] ?? 55))) {
					$lang_warnings[] = 'лид не попадает в рабочий редакционный диапазон';
					$score -= 5;
				}
			}
			...
			if ($content !== '') {
				$content_words = self::word_count($content);
				$min_words = (int) ($formatRules['content_min_words'] ?? 300);
				$max_words = (int) ($formatRules['content_max_words'] ?? 800);
				if ($content_words < $min_words) {
					$lang_warnings[] = 'материал короче ожидаемого редакционного диапазона для этого формата';
					$score -= 8;
				} elseif ($content_words > $max_words) {
					$lang_warnings[] = 'материал выглядит раздутым относительно своего формата';
					$score -= 4;
				}
			}
			if (preg_match('/\b(ситуация продолжает развиваться|международное сообщество внимательно следит|experts say the situation is complex|lage bleibt angespannt|situation remains tense|what happens next)\b/iu', $combined_lower) === 1) {
				$lang_warnings[] = 'в тексте есть пустые шаблонные формулы вместо фактуры';
				$score -= 8;
			}
			$war_tone_issue = self::war_coverage_tone_issue($payload, $title, $excerpt, $content, $lang);
			if ($war_tone_issue !== '') {
				$lang_warnings[] = $war_tone_issue;
				$score -= 18;
			}
	...
	private static function war_coverage_tone_issue(array $payload, string $title, string $excerpt, string $content, string $lang): string {
		...
		$is_war_story =
			in_array('ukraine', $categories, true)
			|| preg_match('/\b(ukraine|ukrain|kyiv|kiew|krim|crimea|krym|sevastopol|sewastopol|russische armee|russische streitkr[aä]fte|russische milit[aä]r|russische aggression|aggression russlands|krieg gegen die ukraine|war against ukraine)\b/u', $text) === 1;
		if (! $is_war_story) {
			return '';
		}
		$aggressor_military_case =
			preg_match('/\b(russische armee|russische streitkr[aä]fte|russische milit[aä]r|russische ziele|russische anlagen|milit[aä]rflugplatz|erd[oö]lanlagen|strategische werke|drohnenlager|radarstation)\b/u', $text) === 1;
		if (! $aggressor_military_case) {
			return '';
		}
		if (preg_match('/\b(tragisch|tragödie|tragisch(?:er|e|es)? verlust|tragic loss|mourning|trauert|beklagt|beklagte|grieving|condolence|beileid|mitgefühl|deeply regrettable|deep regret)\b/u', $text) === 1) {
			return 'для удара по российским военным целям выбран недопустимо сочувствующий тон';
		}
		if (preg_match('/\b(beide seiten|both sides|auf beiden seiten|symmetrisch|escalation on both sides)\b/u', $text) === 1) {
			return 'для сюжета о российской агрессии текст создаёт ложную симметрию сторон';
		}
		return '';
	}
	...
	public static function release_quality(array $payload): array {
		...
		if ($featured === '') {
			...
			$score -= 25;
		} elseif (EPV2_Media::is_generated_story_cover_url($featured)) {
			$warnings[] = 'generated cover не считается publish-grade media';
			$score -= 30;
	...
	private static function featured_media_passes_release_context(array $payload, string $featured, string $title, string $excerpt, array $categories): bool {
		...
		if (EPV2_Media::is_source_host_media($featured, $dossier)) {
			return true;
		}
	...
	private static function category_fit_issue(array $payload, string $title, string $excerpt, string $content): string {
		...
		$sport_text = preg_match('/\b(fc bayern|bundesliga|champions league|uefa|trainer|match|spieltag|torwart|football|soccer|basketball|hockey|galatasaray|liverpool|tv-rechte|übertragungsrechte|uebertragungsrechte|free-tv|live im tv|live im stream)\b/u', $text) === 1;
	...
	private static function stock_fallback_not_allowed(array $payload, string $title, string $excerpt, string $content): bool {
		...
		if (
			$primary === 'kultur'
			&& preg_match('/\b(tatort|fernsehgeschichte|krimireihe|serie|fanpremiere|schauspieler|tv-show|fernsehen|film|kino|batic|leitmayr)\b/u', $text) === 1
		) {
			return false;
		}
	...
	public static function google_preflight_quality(array $payload): array {
		...
		if (trim((string) ($payload['featured_media_url'] ?? $payload['media_url'] ?? '')) === '') {
			...
			$score -= 12;
		} elseif (trim((string) ($payload['media_origin_url'] ?? '')) === '') {
			$warnings[] = 'нет origin-ссылки для главного изображения';
			$score -= 6;
		}
		return [
			'score' => max(0, min(100, $score)),
	...
	private static function publication_format_rules(array $payload, string $lang): array {
		$profile = self::story_budget_profile($payload);
		$shape = sanitize_key((string) ($profile['de']['shape'] ?? 'news'));
		$rules = match ($shape) {
			'bulletin' => ['content_min_words' => 220, 'content_max_words' => 520],
			'service_note' => ['content_min_words' => 280, 'content_max_words' => 700],
			'preview' => ['content_min_words' => 380, 'content_max_words' => 900],
			'article' => ['content_min_words' => 450, 'content_max_words' => 1000],
			'developing' => ['content_min_words' => 700, 'content_max_words' => 1300],
			'analysis' => ['content_min_words' => 1200, 'content_max_words' => 2400],
			default => ['content_min_words' => 300, 'content_max_words' => 850],
		};
		$is_de = $lang === 'de';
		return array_merge($rules, [
			'title_min_words' => 6,
			'title_max_words' => $is_de ? 14 : 16,
			'lead_min_words' => 25,
			'lead_max_words' => $is_de ? 55 : 60,
		]);
	}
	...
	private static function headline_looks_generic(string $title): bool {
		...
		return preg_match('/\b(situation|lage|konflikt|thema|entwicklungen?|news|ticker|liveblog|aktuelle lage|latest updates|situation remains|entwickelt sich weiter)\b/iu', $title) === 1
			&& preg_match('/\b(reject|den(?:ies|y)|weist|bestreitet|announces?|attack|strike|votes?|approves?|kritisiert|warnt|erklärt|meldet)\b/iu', $title) !== 1;
	}
	...
	private static function seo_title_from_title(string $title, string $lang): string {
		...
		return self::trim_chars($title, 68);
	}
