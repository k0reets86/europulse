...
final class EPV2_Admin {
	...
	public static function settings(): void {
		...
		self::row('Мягкий лимит AI-запросов в день', '<input name="ai_daily_request_soft_limit" type="number" min="1" max="1000" value="' . (int) $settings['ai_daily_request_soft_limit'] . '" class="small-text"> <span class="description">Макс. 1000.</span>');
		self::row('Мягкий лимит AI-токенов в день', '<input name="ai_daily_token_soft_limit" type="number" min="1000" max="12000000" step="1000" value="' . (int) $settings['ai_daily_token_soft_limit'] . '" class="regular-text"> <span class="description">Макс. 12 000 000.</span>');
	...
	private static function manual_review_action_hint(object $item): string {
		...
		// Editorial verdicts come first — same wording as rejected,
		// just framed as "selector said no, but it's still here for
		// you because importance was high enough".
		if (in_array('selection_reject', $blockers, true) || $selection_decision === 'reject') {
			return 'Не наша тема: редакторский фильтр счёл материал нерелевантным для нашей аудитории. Если темa всё-таки нужна — нажми "⚡ Опубликовать всё равно". Если фильтр прав — "Удалить".';
		}
		if (in_array('selection_low', $blockers, true) || $selection_decision === 'low') {
			return 'Маловажная для нашей повестки тема. Если важно — нажми "⚡ Опубликовать всё равно". Иначе удали.';
		}
		// Content blockers — these need actual operator action.
		if (in_array('media_contract', $blockers, true)) {
			return 'Нет подходящего фото — у источника не нашлось своего, а сток-картинка для этой темы слабая. Открой материал и приложи фото вручную, либо удали, если новость без картинки не нужна.';
		}
		if (in_array('sources_below_kind_minimum', $blockers, true) || in_array('enrichment_required', $blockers, true)) {
			return 'Мало источников по теме — Bing не нашёл другие издания. Можно нажать "⚡ Опубликовать всё равно" (засчитается одна первичная), либо удалить.';
		}
		if (in_array('payload_contract', $blockers, true) || in_array('stage_contract', $blockers, true)) {
			return 'Пустышка: AI не вытащил из источника достаточно содержания (короткие или пустые тексты, ломаный перевод). Открой и посмотри — если правда нечего публиковать, удали.';
		}
		if (in_array('quality_contract', $blockers, true)) {
			return 'Текст получился слабый по качеству. Открой, прочитай, нажми "Регенерировать" чтобы переписать заново или удали.';
		}
		// Workflow / infra issues — generally re-runnable.
		if ($reason === 'stage_attempt_limit_unknown') {
			return 'Сбой обработки (worker не довёл материал до конца). Нажми "Регенерировать", AI прогонит заново. Если повторится — открой и посмотри источник.';
		}
		if (str_starts_with($reason, 'stage_attempt_limit_')) {
			$stage_name = substr($reason, strlen('stage_attempt_limit_'));
			$stage_label = match ($stage_name) {
				'build_de_master' => 'не получилось собрать немецкий текст',
				'rebuild_bundle' => 'не получилось пересобрать пакет',
				'publish_finish' => 'не получилось довести до публикации',
				'finalize_media' => 'не получилось привязать фото',
				default => 'не сошлась стадия "' . $stage_name . '"',
			};
			return sprintf(
				'%s после двух попыток. Открой и посмотри: если технический сбой — нажми "Регенерировать", если содержательный — удали.',
				ucfirst($stage_label)
			);
		}
		return 'Не пошло автоматом по непонятной причине. Открой и посмотри — после этого либо опубликуй, либо удали, либо регенерируй.';
	}
